-- =============================================================================
-- Infomii — Supabase SQL Editor 用（そのまま貼り付けて実行）
--
-- 内容:
--   1) チーム権限に admin を追加（hotel_memberships / hotel_invites）
--   2) redeem_hotel_invite を admin 対応に更新
--   3) publish_approval_requests テーブル + RLS
--
-- 前提（既にプロジェクトにある通常の SaaS 用テーブル）:
--   public.hotels, public.informations, public.hotel_memberships,
--   public.hotel_invites, public.hotel_invites の招待関連カラム
--
-- 元ファイル: migrations/20260317000001_team_roles.sql の一部概念 +
--            migrations/20260406000100_business_team_admin_and_publish_approval.sql
--
-- 実行後: ダッシュボードの「公開申請一覧」500 エラーが解消する想定です。
-- =============================================================================

-- -----------------------------------------------------------------------------
-- A) チーム role カラム（未追加なら追加）— 先に editor/viewer のみの制約の場合あり
-- -----------------------------------------------------------------------------
...
-- 既存の role 用 CHECK を外して、admin を含む定義に差し替え
alter table public.hotel_memberships
...
-- -----------------------------------------------------------------------------
-- B) redeem_hotel_invite（admin 対応）
-- -----------------------------------------------------------------------------
