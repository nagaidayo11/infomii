...
create table if not exists public.subscriptions (
  ...
  billing_provider text check (billing_provider is null or billing_provider in ('stripe', 'apple')),
  apple_original_transaction_id text,
  apple_product_id text,
  apple_environment text check (apple_environment is null or apple_environment in ('Sandbox', 'Production')),
...
-- Profiles (表示名) — 詳細は migrations/20260423120000_profiles.sql
