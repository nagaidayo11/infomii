...
**App Store の Expo Go は SDK 56 非対応のため、UI 開発はブラウザで行うのが最も速いです。**

1. リポジトリルートで `npm run dev`
2. ブラウザで開く（スマホ幅は DevTools のデバイスモード）:

| 画面 | URL |
|------|-----|
| ダッシュボード（5タブ） | http://127.0.0.1:3000/dashboard?client=app |
| ページ一覧 | http://127.0.0.1:3000/dashboard/pages?client=app |
| テンプレート | http://127.0.0.1:3000/templates?client=app |
| プラン | http://127.0.0.1:3000/settings/billing?client=app |
| 設定 | http://127.0.0.1:3000/settings?client=app |

通常 Web UI に戻す: `?client=web` または `?client=app` を外す。

実機アプリの確認はストア提出前に `npx expo run:ios` または EAS Build で行います。
...
- **Expo Go「requires newer version」**: 実機 App Store 版は SDK 54 相当。SDK 56 プロジェクトは Expo Go 不可 → 上記ブラウザ `?client=app` か `expo run:ios`。
