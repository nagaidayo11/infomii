    ...
    <>
      ...
      <div className="mt-3 space-y-2.5">
        {items.map((item, index) => {
          ...
          const duration = getLocalizedContent(item.duration as LocalizedString | undefined, locale);
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="font-semibold leading-snug text-slate-900" style={getBodyFontSizeStyle()}>
                  {name}
                  {duration ? <span className="ml-2 font-normal text-slate-500">（{duration}）</span> : null}
                  {price ? <span className="ml-1 font-semibold text-slate-800">{price}</span> : null}
                </p>
                {description ? (
