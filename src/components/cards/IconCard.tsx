      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <p className="font-semibold text-slate-900" style={getTitleFontSizeStyle()}>
            <InlineEditable value={label} onSave={(v) => updateKey("label", v)} editable={isSelected} onActivate={onActivate} placeholder="ラベル" className="font-semibold text-slate-900" />
          </p>
          <p className="mt-1 text-slate-600" style={getBodyFontSizeStyle()}>
            <InlineEditable value={description} onSave={(v) => updateKey("description", v)} editable={isSelected} onActivate={onActivate} placeholder="補足" className="text-slate-600" />
          </p>
        </div>
      </div>
    </Card>
