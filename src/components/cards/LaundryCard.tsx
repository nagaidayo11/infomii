  ...
  if (isNativeUi) {
    return (
      <NativeHotelSection title={titleNode} icon={<NativeClockIcon />} onActivate={onActivate}>
        <NativeKvList>
          <NativeKvRow label={labels.hours}>
            <InlineEditable
              value={hours}
              onSave={(v) => updateKey("hours", v)}
              editable={editable}
              onActivate={onActivate}
              placeholder="9:00–18:00"
            />
          </NativeKvRow>
          <NativeKvRow label={labels.contact}>
            <InlineEditable
              value={contact}
              onSave={(v) => updateKey("contact", v)}
              editable={editable}
              onActivate={onActivate}
        ...
        {(isSelected || priceNote.trim().length > 0 || editable) ? (
          <p className="mt-2 text-sm text-[var(--app-text-muted)]">
            <InlineEditable
              value={priceNote}
              onSave={(v) => updateKey("priceNote", v)}
              editable={editable}
              onActivate={onActivate}
              className="text-sm text-[var(--app-text-muted)]"
              placeholder={labels.priceNotePlaceholder}
            />
          </p>
        ) : null}
      </NativeHotelSection>
  ...
  return (
    <Card padding="md" className="">
      ...
      <p className="text-slate-600" style={getBodyFontSizeStyle()}>
        {labels.hours}:{" "}
        <InlineEditable value={hours} onSave={(v) => updateKey("hours", v)} editable={editable} onActivate={onActivate} className="text-slate-600" placeholder="9:00–18:00" />
      </p>
      <p className="text-slate-500" style={getBodyFontSizeStyle()}>
        <InlineEditable value={priceNote} onSave={(v) => updateKey("priceNote", v)} editable={editable} onActivate={onActivate} className="text-slate-500" placeholder={labels.priceNotePlaceholder} />
      </p>
      <p className="text-slate-600" style={getBodyFontSizeStyle()}>
        {labels.contact}:{" "}
        <InlineEditable value={contact} onSave={(v) => updateKey("contact", v)} editable={editable} onActivate={onActivate} className="text-slate-600" placeholder={labels.contactPlaceholder} />
      </p>
      </div>
    </Card>
  );
