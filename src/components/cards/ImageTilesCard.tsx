        ...
        {items.slice(0, 12).map((item, i) => {
          ...
          const tile = (
            <>
              <div
                data-inner-surface
                className={`relative aspect-[4/3] overflow-hidden ${editorInnerRadiusClassName} bg-slate-100`}
              >
                {item?.src ? (
                  <EditorCoverImage
                    src={item.src}
              ...
              <div className="mt-1.5 px-0.5">
                {editable ? (
                  ...
                  <p className="truncate text-sm font-medium text-slate-800" style={getBodyFontSizeStyle()}>
                    {label}
                  </p>
                )}
              </div>
