...
export function LiveOpsQuickOps({ opsKey }: { opsKey: LiveOpsKey }) {
  ...
  const [hotelTargets, setHotelTargets] = useState<LiveOpsTarget[]>([]);
  const [pageTargets, setPageTargets] = useState<LiveOpsTarget[]>([]);
  const [pageTitle, setPageTitle] = useState<string | null>(null);
  const [pageMissing, setPageMissing] = useState(false);
  const [activeCardId, setActiveCardId] = useState<string | null>(null);
  const [level, setLevel] = useState<LiveOpsLevel>("open");
  ...
  const applyTarget = useCallback((target: LiveOpsTarget | null) => {
    if (!target) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [editable, allTargets] = await Promise.all([
        canEditLiveOps(),
        listHotelLiveOpsTargets(opsKey),
      ]);
      setCanEdit(editable);
      ...
      if (!resolvedPageId) {
        const last = readLastLiveOpsPageId(opsKey);
      ...
      if (pageIdParam !== resolvedPageId && !pageIdParam) {
        router.replace(buildLiveOpsHref(opsKey, resolvedPageId));
      }

      rememberLiveOpsPageId(opsKey, resolvedPageId);
      const { page, targets } = await loadLiveOpsTargetsForPage(opsKey, resolvedPageId);
      if (!page) {
        setPageMissing(true);
        setPageTargets([]);
        setPageTitle(null);
  ...
  }, [opsKey, pageIdParam, router, applyTarget]);
  ...
  async function handleSave() {
    ...
    setError(null);
    try {
      const next = await saveLiveOpsStatus(activeTarget.page.id, opsKey, { level, note });
      setUpdatedAt(next.updatedAt);
      // Page-level ops: every matching card on this page shares the same status.
      ...
      const msg = def.saveSuccessMessage;
      setSaveNotice(msg);
  ...
  function handlePageChange(nextPageId: string) {
    rememberLiveOpsPageId(opsKey, nextPageId);
    router.push(buildLiveOpsHref(opsKey, nextPageId));
  }

  const updatedLabel = formatLiveOpsUpdatedAt(updatedAt) ?? "最終更新 —";
  const pagesWithOps = useMemo(() => {
  ...
  return (
    <div className="app-main-container mx-auto max-w-lg space-y-5 pb-10">
      <header className="app-page-header">
        ...
        <h1 className="mt-1 app-page-title">{def.quickOpsTitle}</h1>
        <p className="app-page-subtitle">{def.quickOpsSubtitle}</p>
      </header>
      ...
      {loading ? (
        ...
        <EmptyBlock
          ...
          actionLabel="ページ一覧へ"
        />
      ) : pagesWithOps.length === 0 ? (
        <EmptyBlock
          title={def.emptyNoBlocksTitle}
          body={def.emptyNoBlocksBody}
        ...
        <>
          <section className="space-y-2">
            ...
            <select
              id="ops-page"
              value={pageIdParam ?? pagesWithOps[0]?.id ?? ""}
          ...
          {pageTargets.length === 0 ? (
            <EmptyBlock
              title={def.emptyPageNoBlocksTitle}
              body={def.emptyPageNoBlocksBody}
