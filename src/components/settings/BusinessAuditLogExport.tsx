...
import { AppSettingsGroup } from "@/components/app-shell/AppSettingsGroup";
...
export function BusinessAuditLogExport() {
  ...
  const card = (
    <AppSettingsCard className={isAppShell ? "app-settings-audit-card" : ""}>
      {isAppShell ? (
        <button
          type="button"
          disabled={busy}
          onClick={() => void handleExport()}
          className="app-settings-audit-row app-pressable ui-pop-tap"
        >
          <span className="app-settings-audit-row-label">監査ログをエクスポート</span>
          <span className="app-settings-audit-row-action">{busy ? "作成中…" : "CSV"}</span>
        </button>
      ) : (
        <>
  ...
  if (isAppShell) {
    return (
      <AppSettingsGroup
        title="Business"
        footer={`操作履歴を最大 ${AUDIT_EXPORT_LIMIT} 件まで CSV で保存できます。`}
      >
        {card}
      </AppSettingsGroup>
    );
